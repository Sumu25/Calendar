...
<?php
    ...
    //adding new event
    if ($_POST) {
        //creating database-safe strings
        ...
        $safe_event_title = mysqli_real_escape_string($mysqli,
        $_POST['event_title']);
        $safe_event_shortdesc = mysqli_real_escape_string($mysqli,
        $_POST['event_shortdesc']);
        $safe_event_time_hh = mysqli_real_escape_string($mysqli,
        $_POST['event_time_hh']);
        $safe_event_time_mm = mysqli_real_escape_string($mysqli,
        $_POST['event_time_mm']);

        $event_date = $safe_y."-".$safe_m."-".$safe_d." ".$safe_event_time_hh.":".$safe_event_time_mm.":00";
        echo "date:".$event_date;
        $intEvent_sql = "INSERT INTO calendar_events(event_title,event_shortdesc,event_start) VALUES ('".$safe_event_title."', '".$safe_event_shortdesc."','".$event_date."')";
        $intEvent_res = mysqli_query($mysqli,$intEvent_sql) or die(mysqli_error($mysqli));
    }
    else {
    ...
    //Showing form for adding an event
    echo <<<END_OF_TEXT
